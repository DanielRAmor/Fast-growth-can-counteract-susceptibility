...
colnames(phaseX) = c('alpha12', 'Ks', 'xf','yf')
...
#plot the results
ploti <- ggplot(data = phaseX, aes(x= alpha12, y= Ks ))
...
titulos<- labs(title="", x=expression(paste("Inhibition of F by S (Alpha_fs)")), y=expression(paste("Carrying Capacity of S")))
...
escalaY <- scale_y_continuous(expand= c(0,0), limits=c(KMin,KMax)) # coord_cartesian(expand= c(0,0), ylim = c(0,7)) 
...
plotii
#export plot
pdf(paste("F_S4C.pdf"), width=6, height=4.5)
print(plotii)
dev.off()
