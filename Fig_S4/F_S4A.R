...
#set range of Alpha12 an Ks to explore
AlphaMax <- 3.0
AlphaMin <- 0.3
a12 = AlphaMax
a21 = AlphaMax
...
colnames(phaseX) = c('alpha12', 'alpha21', 'xf','yf')


#begin simulations to plot the phase space
yini = c(0.1, 0.1)
Nsims = 201 #resolution of each axis of the phase space (number of simulations in each direction of the phase space)
#'for' loop to explore the range of alpha12 and Ks

for (i in 1:Nsims){
  print(i) #to report progress in the 'for' loop
  if (i > 1) {a12 = a12 - (AlphaMax-AlphaMin)/(Nsims-1)}  
  
  for (j in 1:Nsims) {
    if (j < 2) {a21 = AlphaMax
    } else {
      a21 = a21 - (AlphaMax-AlphaMin)/(Nsims-1)  
    }
    ...
    phaseX[(i-1)*Nsims + j,1] = a12
    phaseX[(i-1)*Nsims + j,2] = a21
...
#plot the results
ploti <- ggplot(data = phaseX, aes(x= alpha12, y= alpha21 ))
...
titulos<- labs(title="", x=expression(paste("Inhibition of F by S (Alpha_fs)")), y=expression(paste("Inhibition of S by F (Alpha_sf)")))
escalaX <- scale_x_continuous(expand= c(0,0), limits=c(AlphaMin,AlphaMax))#, breaks=seq(0, 0.8, 0.2)) # coord_cartesian(expand= c(0,0), xlim = c(0, 1.3)) 
escalaY <- scale_y_continuous(expand= c(0,0), limits=c(AlphaMin,AlphaMax)) # coord_cartesian(expand= c(0,0), ylim = c(0,7)) 
...
plotii
#export plot
pdf(paste("F_S4A.pdf"), width=6, height=4.5)
print(plotii)
dev.off()
