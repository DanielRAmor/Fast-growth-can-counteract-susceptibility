...
Day2Lp_1X_in_CaS <- read.table ('Day2_Lp_in_1over8_CaSupernatant.dat', header=F, sep = "\t", dec = ".")
...
Day2Lp_1X_in_CaS[1] <- c('NaN', 'NaN', 'NaN', 'Amp', 'NaN', 'Car', 'NaN', 'NaN')
...
#Plot
Plot1XLp <- ggplot(subset(Stats1XLp_in_Ca, Antibiotic != 'NaN'), aes(x= Supernatant, y= MeanOD/LpMeanK, group= Supernatant))
...
escalaY <- scale_y_continuous(breaks = pretty(Stats1XLp_in_Ca$MeanOD/LpMeanK, n = 3), limits = c(0, NA))
...
plotii = Plot1XLp + background  + titulos + couleur + paneles + puntos + barras + escalaY
plotii

pdf("F_S13G.pdf", width=2.5, height=3)
print(plotii)
dev.off()
