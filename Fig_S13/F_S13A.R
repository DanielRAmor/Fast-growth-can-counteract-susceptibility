# Analisys on experimental data to assess self-protective interactions via culturing the Ca species 
#in its own supernatant in the presence of antibiotics.
...
#OD Ca cells in 1X antibiotics, Ca Supernatant
Day2Ca_1X_in_CaS <- read.table ('Day2_Ca_in_1X_CaSupernatant.dat', header=F, sep = "\t", dec = ".")
colnames(Day2Ca_1X_in_CaS) <- c('Antibiotic','30X_1', '30X_2','30X_3','HighD_1','HighD_2','HighD_3','LowD_1','LowD_2','LowD_3','OnlyAnt_1','OnlyAnt_2', 'OnlyAnt_3')
Day2Ca_1X_in_CaS[1] <- c('Ery', 'Chl', 'Gen', 'Amp', 'Kan', 'Car', 'Tet', 'Cip')
#Subtract background OD for each antibiotic
for (i in 2:13){
  Day2Ca_1X_in_CaS[i] <- Day2Ca_1X_in_CaS[i] - Day2Background[14]
}

Ca_1X_in_CaS <-  as.data.frame( melt(Day2Ca_1X_in_CaS,  id.vars=c('Antibiotic'),  value.name="OD")) #formatting
#add replicate indexing
Ca_1X_in_CaS[,4] <- substr(Ca_1X_in_CaS[,2], nchar(as.character(Ca_1X_in_CaS[,2])),nchar(as.character(Ca_1X_in_CaS[,2])))
#remove replicate indexing from column 2
Ca_1X_in_CaS[,2] <- substr(Ca_1X_in_CaS[,2], 1,nchar(as.character(Ca_1X_in_CaS[,2]))-2)
colnames(Ca_1X_in_CaS) <- c('Antibiotic','Supernatant', 'OD', 'Replicate')

#Ca cells in 1X Ca supernatant
#Mean and StError for each Antibiotic&Supernatant condition
AntibClass <- unlist(Ca_1X_in_CaS[1] %>% unique)  # unique is creating a list, but we need a vector, so we also use unlist()
SupernClass <- unlist(dplyr::distinct(Ca_1X_in_CaS[2])) #alternative way of getting unique values
Stats1XCa_in_Ca <- data.frame(Antibiotic = c('Chl','Chl'),Supernatant=c('HighD','LowD'), MeanOD=c(0.1,0.5),StErrOD=c(0.01,0.02))
replicates <- 3  
for (anti in 1:length(AntibClass)){
  for (super in 1:length(SupernClass)){
    Stats1XCa_in_Ca[super+(anti-1)*length(SupernClass),1] <- AntibClass[anti]
    Stats1XCa_in_Ca[super+(anti-1)*length(SupernClass),2] <- SupernClass[super]
    Stats1XCa_in_Ca[super+(anti-1)*length(SupernClass),3] <- mean(Ca_1X_in_CaS[Ca_1X_in_CaS[,1]== AntibClass[anti] & Ca_1X_in_CaS[,2]== SupernClass[super],3])
    Stats1XCa_in_Ca[super+(anti-1)*length(SupernClass),4] <-   sd(Ca_1X_in_CaS[Ca_1X_in_CaS[,1]== AntibClass[anti] & Ca_1X_in_CaS[,2]== SupernClass[super],3])/sqrt(replicates)
  }
} 


#Plot
Plot1XCa <- ggplot(subset(Stats1XCa_in_Ca), aes(x= Supernatant, y= MeanOD/CaMeanK, group= Supernatant))
...
escalaY <- scale_y_continuous(breaks = pretty(Stats1XCa_in_Ca$MeanOD/CaMeanK, n = 2), limits = c(0, NA))
escalaY <- scale_y_continuous(breaks = pretty(Stats1XCa_in_Ca$MeanOD/CaMeanK, n = 2))
...
plotii = Plot1XCa + background  + titulos + couleur + paneles + puntos + barras
plotii

pdf("F_S13A.pdf", width=2.5, height=10)
print(plotii)
dev.off()
