# Analisys on experimental data to assess self-protective interactions via culturing the Lp species 
...
#OD Lp cells in 1X antibiotics, Lp Supernatant
Day2Lp_1X_in_LpS <- read.table ('Day2_Lp_in_1X_LpSupernatant.dat', header=F, sep = "\t", dec = ".")
colnames(Day2Lp_1X_in_LpS) <- c('Antibiotic','30X_1', '30X_2','30X_3','HighD_1','HighD_2','HighD_3','LowD_1','LowD_2','LowD_3','OnlyAnt_1','OnlyAnt_2', 'OnlyAnt_3')
Day2Lp_1X_in_LpS[1] <- c('Ery', 'Chl', 'Gen', 'Amp', 'Kan', 'Car', 'Tet', 'Cip')
#Subtract background OD for each antibiotic
for (i in 2:13){
  Day2Lp_1X_in_LpS[i] <- Day2Lp_1X_in_LpS[i] - Day2Background[14]
}

Lp_1X_in_LpS <-  as.data.frame( melt(Day2Lp_1X_in_LpS,  id.vars=c('Antibiotic'),  value.name="OD")) #formatting
#add replicate indexing
Lp_1X_in_LpS[,4] <- substr(Lp_1X_in_LpS[,2], nchar(as.character(Lp_1X_in_LpS[,2])),nchar(as.character(Lp_1X_in_LpS[,2])))
#remove replicate indexing from column 2
Lp_1X_in_LpS[,2] <- substr(Lp_1X_in_LpS[,2], 1,nchar(as.character(Lp_1X_in_LpS[,2]))-2)
colnames(Lp_1X_in_LpS) <- c('Antibiotic','Supernatant', 'OD', 'Replicate')

#Lp cells in 1X Lp supernatant
#Mean and StError for each Antibiotic&Supernatant condition
AntibClass <- unlist(Lp_1X_in_LpS[1] %>% unique)  # unique is creating a list, but we need a vector, so we also use unlist()
SupernClass <- unlist(dplyr::distinct(Lp_1X_in_LpS[2])) #alternative way of getting unique values
Stats1XLp_in_Lp <- data.frame(Antibiotic = c('Chl','Chl'),Supernatant=c('HighD','LowD'), MeanOD=c(0.1,0.5),StErrOD=c(0.01,0.02))
replicates <- 3  
for (anti in 1:length(AntibClass)){
  for (super in 1:length(SupernClass)){
    Stats1XLp_in_Lp[super+(anti-1)*length(SupernClass),1] <- AntibClass[anti]
    Stats1XLp_in_Lp[super+(anti-1)*length(SupernClass),2] <- SupernClass[super]
    Stats1XLp_in_Lp[super+(anti-1)*length(SupernClass),3] <- mean(Lp_1X_in_LpS[Lp_1X_in_LpS[,1]== AntibClass[anti] & Lp_1X_in_LpS[,2]== SupernClass[super],3])
    Stats1XLp_in_Lp[super+(anti-1)*length(SupernClass),4] <-   sd(Lp_1X_in_LpS[Lp_1X_in_LpS[,1]== AntibClass[anti] & Lp_1X_in_LpS[,2]== SupernClass[super],3])/sqrt(replicates)
  }
} 


#Plot
Plot1XLp <- ggplot(subset(Stats1XLp_in_Lp), aes(x= Supernatant, y= MeanOD/LpMeanK, group= Supernatant))
...
escalaY <- scale_y_continuous(breaks = pretty(Stats1XLp_in_Lp$MeanOD/LpMeanK, n = 2), limits = c(0, NA))
escalaY <- scale_y_continuous(breaks = pretty(Stats1XLp_in_Lp$MeanOD/LpMeanK, n = 2))
...
pdf("F_S13D.pdf", width=2.5, height=10)
print(plotii)
dev.off()
