...
#in Lp's supernatant in the presence of antibiotics.
...
Day2Ca_1X_in_LpS <- read.table ('Day2_Ca_in_1X_LpSupernatant.dat', header=F, sep = "\t", dec = ".")
...
Day2Ca_1X_in_LpS[1] <- c('Ery', 'Chl', 'Gen', 'Amp', 'Kan', 'Car', 'Tet', 'Cip')
...
#Plot
Plot1XCa <- ggplot(subset(Stats1XCa_in_Lp), aes(x= Supernatant, y= MeanOD/CaMeanK, group= Supernatant))
...
escalaY <- scale_y_continuous(breaks = pretty(Stats1XCa_in_Lp$MeanOD/CaMeanK, n = 2), limits = c(0, NA))
escalaY <- scale_y_continuous(breaks = pretty(Stats1XCa_in_Lp$MeanOD/CaMeanK, n = 2))
...
pdf("F_S13B.pdf", width=2.5, height=10)
print(plotii)
dev.off()
