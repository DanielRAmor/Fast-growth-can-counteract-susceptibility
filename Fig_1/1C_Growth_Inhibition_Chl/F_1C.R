...
#The algorithm analyzes and prepares the data for measuring and plotting the susceptibility to the antibiotic of interest (here, chloramphenicol)
...
#above, the data was corrected for the background OD
...
#plot aesthetics
background <- theme_bw() + theme ( panel.grid.minor=element_blank(), panel.grid.major=element_blank(), axis.line= element_blank(), panel.border = element_rect (colour="black", size=1.0), axis.text=element_text(size=9), axis.title=element_text(size=11), title=element_text(size=12)) #, axis.title.x=element_blank() )
titulos <- labs(title="", x=expression(paste("Concentration")), y=expression(paste("Normalized_OD")))
...
#Normalization, CODE CHANGES WITH CONDITION!
maxCa24 <- mean (G9[G9$Species == 'Ca' & G9$Time_hr > 23 & G9$Well == 'DB12', 5])
maxLp24 <- mean (G9[G9$Species == 'Lp' & G9$Time_hr > 23 & G9$Well == 'DB12', 5])
...
# change well labels to corresponding antibiotic concentration
xlabels <- scale_x_discrete(limits = unique(rev(G9$Well)),labels=c("DB12" = "0.0", "DB11" = ".10","DB10" = "0.20", "D9" = ".39", "D8" = ".78", 
                                                                   "D7" = "1.6","D6" = "3.1","D5" = "6.3","D4" = "12.5","D3" = "25","D2" = "50","D1" = "100"))
...
pdf(paste(  "F_1C", ".pdf"), width=6, height=4)
print(plotii)
dev.off()
